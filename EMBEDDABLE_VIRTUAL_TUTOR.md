...
-------- | ----------- | --------------
`<LANGUAGE_CODE>` (Optional) | Specifies the language of the Embeddable Virtual Tutor's interface. This does not affect the response language of the Virtual Tutor. | `de` (German), `en` (English), `es` (Espagnol), `fr` (French) or `it` (Italian)
...
`<TOKEN>` (Required) | The login token generated for your users to securely access the Embeddable Virtual Tutor. | Must be generated using the public API [genTokenForEmbeddableVirtualTutorGuest](https://github.com/qurami/smart-notes-api-quickstart#generate-a-token-for-external-users-to-access-the-embeddable-virtual-tutor).
